...
import { Alert, Autocomplete, Box, Button, Card, Checkbox, Dialog, DialogActions, DialogContent, DialogTitle, Divider, FormControlLabel, MenuItem, Stack, Tab, Tabs, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TextField, Typography } from '@mui/material';
...
type AssignmentOption = { id: number; label: string; detail?: string };

function AssignmentMultiSelect({ label, options, value, onChange, helperText }: { label: string; options: AssignmentOption[]; value: number[]; onChange: (ids: number[]) => void; helperText: string }) {
  const selected = options.filter(option => value.includes(option.id));
  return <Autocomplete multiple disableCloseOnSelect limitTags={2} options={options} value={selected} isOptionEqualToValue={(option, current) => option.id === current.id} getOptionLabel={option => option.label} onChange={(_, next) => onChange(next.map(option => option.id))} renderOption={(props, option, state) => <Box component="li" {...props} key={option.id} sx={{ display: 'flex', gap: 1 }}><Checkbox checked={state.selected} size="small" sx={{ p: .5 }} /><Box><Typography variant="body2">{option.label}</Typography>{option.detail && <Typography variant="caption" color="text.secondary">{option.detail}</Typography>}</Box></Box>} renderInput={params => <TextField {...params} label={label} helperText={helperText} placeholder={selected.length ? '' : `Select ${label.toLowerCase()}`} />} />;
}
...
export function AdminUsers({ superadmin, mode = 'users' }: { superadmin: boolean; mode?: 'users' | 'classes' }) {
  ...
  const userTable = useServerTable<ManagedUser>({ path: '/users/', initialSort: 'user', extraParams: `role=${roleForTab}${studentFilters}` });
  ...
  return <>
    ...
    <Dialog component="form" open={editingUser !== undefined} onClose={() => setEditingUser(undefined)} onSubmit={event => { event.preventDefault(); void saveUser(); }} fullWidth maxWidth="sm">
      <DialogTitle>{editingUser ? `Manage ${userForm.role}` : `Add ${roleForTab}`}</DialogTitle><DialogContent><Stack gap={2} sx={{ pt: 1 }}>
        ...
        <TextField label="Role" value={userForm.role === 'teacher' ? 'Teacher / ARAL Tutor' : userForm.role === 'admin' ? 'Administrator' : 'Student'} disabled helperText="The account type is determined by the selected user-management tab." />
        ...
        {userForm.role === 'student' && <><TextField select label="Grade Level" value={formGrade} onChange={event => { const next = Number(event.target.value); setFormGrade(next); setUserForm(value => ({ ...value, academic_class: '' })); }}><MenuItem value={11}>Grade 11</MenuItem><MenuItem value={12}>Grade 12</MenuItem></TextField><TextField select label="Class" value={userForm.academic_class} onChange={event => setUserForm(value => ({ ...value, academic_class: Number(event.target.value) }))} required helperText={`Only Grade ${formGrade} classes are available.`}>{classes.filter(item => item.is_active && item.grade_level === formGrade).map(item => <MenuItem key={item.id} value={item.id}>{item.name}</MenuItem>)}</TextField></>}
        {userForm.role === 'teacher' && <><AssignmentMultiSelect label="Assigned Subjects" options={subjectOptions} value={userForm.assigned_subjects} onChange={assigned_subjects => setUserForm(value => ({ ...value, assigned_subjects }))} helperText="Each subject determines its grade level and learner scope." /><Box sx={{ p: 2, bgcolor: 'background.default', border: '1px solid', borderColor: 'divider', borderRadius: 1 }}><Typography variant="body2" fontWeight={700}>Derived Class Access</Typography><Typography variant="body2" color="text.secondary" sx={{ mt: .5 }}>{derivedClasses.length ? derivedClasses.map(item => item.label).join(', ') : 'Assign at least one subject to derive class access.'}</Typography></Box></>}
