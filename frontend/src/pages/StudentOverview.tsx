...
export function StudentOverview({ onContinue, onAssessments, onMaterials, onEnrollment }: { onContinue: () => void; onAssessments: () => void; onMaterials: () => void; onEnrollment: () => void }) {
  ...
  useEffect(() => { load(); }, [load]);
  if (scope && !scope.loading && !scope.selectedSubjectId) return <><PageHeader title="My Academic Recovery" /><Alert severity="info" action={<Button onClick={onEnrollment}>Open enrollment</Button>}>You are not enrolled in an ARAL subject yet.</Alert></>;
  ...
  const latestPendingReview = latestAttempt?.grading_status === 'pending_review';
  const latestAllMastered = Boolean(latestAttempt?.competency_results.length) && latestAttempt!.competency_results.every(item => item.status === 'mastered');
  ...
  return <>
    ...
    </Card> : !data!.pending_diagnostic && <Card sx={{ mb: 3, p: { xs: 2.5, sm: 3 } }}><Stack direction={{ xs: 'column', sm: 'row' }} gap={2} alignItems={{ sm: 'center' }}><Box sx={{ width: 44, height: 44, borderRadius: '50%', display: 'grid', placeItems: 'center', bgcolor: latestAllMastered ? 'success.light' : 'primary.light', color: latestAllMastered ? 'success.dark' : 'primary.dark' }}><CheckCircle /></Box><Box sx={{ flex: 1 }}><Typography variant="h3">{latestPendingReview ? 'Assessment awaiting teacher review' : latestAllMastered ? 'No recovery support needed right now' : 'No recovery activities right now'}</Typography><Typography variant="body2" color="text.secondary" sx={{ mt: .5 }}>{latestPendingReview ? 'Your teacher will complete the review before any learning support is assigned.' : latestAllMastered ? 'Your latest results show that no competency currently requires remediation.' : 'There is no recovery activity waiting for you in this subject. Review your assessments or continue with your assigned learning materials.'}</Typography></Box><Button variant="outlined" onClick={onAssessments}>View assessments</Button></Stack></Card>}
