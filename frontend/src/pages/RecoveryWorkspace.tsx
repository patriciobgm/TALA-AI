...
export function RecoveryWorkspace({ onAssessments }: { onAssessments: (competencyId: number) => void }) {
  ...
  const load = useCallback(() => { if (!scope?.selectedSubjectId) return Promise.resolve(); return api<{ results?: ApiPlan[] } | ApiPlan[]>(`/recovery-plans/?subject=${scope.selectedSubjectId}`).then(result => {
    const returned = Array.isArray(result) ? result : result.results ?? [];
