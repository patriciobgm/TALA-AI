...
export function LearnerProfile({ learnerId, onBack }: { learnerId: number; onBack: () => void }) {
  ...
  const exemptRemedial = async (assessmentId: number) => {
    setConsentBusy(assessmentId); setError('');
    try { await api(`/assessments/${assessmentId}/eligibility/`, { method: 'POST', body: JSON.stringify({ student: learnerId, status: 'exempted', reason: 'Teacher determined that a remedial examination is not required.' }) }); setSuccessMessage('Learner marked as not requiring this remedial exam.'); await load(); }
    catch (reason) { setError(reason instanceof Error ? reason.message : 'Unable to update remedial eligibility.'); }
    finally { setConsentBusy(null); }
  };
