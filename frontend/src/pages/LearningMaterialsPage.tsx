...
import { Alert, Box, Button, Card, CircularProgress, Dialog, DialogActions, DialogContent, DialogTitle, Divider, FormControl, FormControlLabel, IconButton, LinearProgress, Radio, RadioGroup, Stack, TextField, Tooltip, Typography } from '@mui/material';
import { CheckOutlined, FitScreenOutlined, MenuBookOutlined, PlayCircleOutline, ZoomInOutlined, ZoomOutOutlined } from '@mui/icons-material';
import { api, normalizeProtectedUrl, protectedBlob } from '../api/client';
...
export function LearningMaterialsPage() {
  ...
  useEffect(() => {
    if (!selected?.file_url || selected.resource_type !== 'module') { setPdfPreviewUrl(''); setPdfError(''); return; }
    let active = true;
    let objectUrl = '';
    setPdfBusy(true); setPdfError('');
    protectedBlob(selected.file_url).then(blob => {
      if (!active) return;
      if (!blob.size) throw new Error('The server returned an empty document.');
      objectUrl = URL.createObjectURL(blob);
      setPdfPreviewUrl(objectUrl);
    }).catch(reason => { if (active) { setPdfPreviewUrl(''); setPdfError(reason instanceof Error ? reason.message : 'Unable to display this PDF.'); } }).finally(() => { if (active) setPdfBusy(false); });
  ...
  const openMaterial = async (assignment: LearningAssignment) => {
    setBusy(true); setError(''); setPdfError(''); setPdfZoom(100); setQuizAnswers({}); setQuizResult(null); setQuizOpen(false); setQuizStarted(false); setFocusedQuestionId(null);
  ...
  const savePlayback = async (video: HTMLVideoElement, force = false) => {
    ...
    try { replaceAssignment(await api<LearningAssignment>(`/learning-assignments/${selected.id}/progress/`, { method: 'POST', body: JSON.stringify({ position_seconds: position, duration_seconds: duration }) })); }
  ...
  const quizAnswered = selected ? selected.practice_questions.every(question => Boolean(quizAnswers[question.id]?.trim())) : false;
  return <>
    ...
    <Card><Stack divider={<Divider />}>
      {ordered.map(item => <Box key={item.id} sx={{ p: 2.5, display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '40px minmax(0, 1fr) auto' }, alignItems: 'center', gap: 2 }}>
        ...
        <Box><Stack direction="row" alignItems="center" gap={1} flexWrap="wrap"><Typography variant="body1" fontWeight={700}>{item.resource_title}</Typography><StatusChip label={item.completed_at ? 'Completed' : item.opened_at ? 'In Progress' : 'Assigned'} />{item.quiz_required && <StatusChip label={item.quiz_passed ? 'Quiz passed' : 'Quiz included'} />}</Stack><Typography variant="body2" color="text.secondary" sx={{ mt: .5 }}>{item.competency ? `${item.competency.code} · ${item.competency.title}` : 'General learning material'}{item.due_at ? ` · Due ${new Date(item.due_at).toLocaleDateString()}` : ''}</Typography><Typography variant="caption" color="text.secondary">Uploaded by {item.uploaded_by_name}</Typography>{item.resource_type === 'video' && item.opened_at && !item.completed_at && <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 1, maxWidth: 360 }}><LinearProgress variant="determinate" value={item.progress_percent} sx={{ flex: 1, height: 6 }} /><Typography variant="caption" color="text.secondary">{item.progress_percent}%</Typography></Box>}{item.instructions && <Typography variant="body2" sx={{ mt: .75 }}>{item.instructions}</Typography>}</Box>
    ...
    {selected && <Dialog open={!quizOpen} onClose={() => !busy && setSelected(null)} fullWidth maxWidth="lg">
      <DialogTitle><Typography variant="overline" color="text.secondary">{selected.resource_type}</Typography><Typography variant="h2">{selected.resource_title}</Typography>{selected.competency && <Typography variant="body2" color="text.secondary">{selected.competency.code} · {selected.competency.title}</Typography>}</DialogTitle>
      <DialogContent dividers sx={{ p: 0 }}>
        {selected.instructions && <Alert severity="info" sx={{ m: 2 }}>{selected.instructions}</Alert>}
        {selected.quiz_required && !selected.quiz_passed && <Alert severity="info" sx={{ m: 2 }}>This material is completed by passing its learning quiz. Review the content first, then choose Take Learning Quiz when you are ready.</Alert>}
        {selected.resource_type === 'video' && selected.file_url ? <Box sx={{ p: 2 }}><video controls preload="metadata" src={selected.file_url} onLoadedMetadata={event => { const video = event.currentTarget; lastSavedSecond.current = selected.playback_position_seconds; if (selected.playback_position_seconds > 0 && selected.playback_position_seconds < video.duration - 2) video.currentTime = selected.playback_position_seconds; }} onTimeUpdate={event => void savePlayback(event.currentTarget)} onPause={event => void savePlayback(event.currentTarget, true)} onEnded={event => { void savePlayback(event.currentTarget, true).then(() => complete()); }} style={{ width: '100%', maxHeight: '68vh', background: '#111', borderRadius: 8 }}><track kind="captions" /></video></Box> : selected.file_url ? <>
      ...
      <DialogActions><Button onClick={() => setSelected(null)} disabled={busy}>Close</Button>{selected.quiz_required ? <Button variant="contained" disabled={busy || selected.quiz_passed} onClick={() => { setQuizStarted(false); setQuizOpen(true); }}>{selected.quiz_passed ? 'Quiz Passed' : 'Take Learning Quiz'}</Button> : <Button variant="contained" startIcon={<CheckOutlined />} onClick={() => void complete()} disabled={busy || Boolean(selected.completed_at)}>{selected.completed_at ? 'Completed' : busy ? 'Saving…' : 'Mark as Completed'}</Button>}</DialogActions>
    </Dialog>}
    {selected?.quiz_required && <Dialog open={quizOpen} onClose={() => !busy && setQuizOpen(false)} fullWidth maxWidth={quizStarted ? 'md' : 'sm'}><DialogTitle>{quizStarted ? 'Learning Quiz' : 'Ready to Take the Learning Quiz?'}</DialogTitle><DialogContent dividers>{!quizStarted ? <Box><Typography variant="body1" fontWeight={700}>{selected.resource_title}</Typography><Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>This quiz contains {selected.practice_questions.length} questions. Review the learning material first, then start when you are ready. Submit only after answering every question.</Typography>{quizResult && !quizResult.passed && <Alert severity="warning" sx={{ mt: 2 }}>Your previous score was {Math.round(quizResult.score)}%. The passing score is {quizResult.required_score}%.</Alert>}</Box> : <>{quizResult?.passed && <Alert severity="success" sx={{ mb: 2 }}>Quiz passed with {Math.round(quizResult.score)}%. The learning material is complete.</Alert>}{quizResult && !quizResult.passed && <Alert severity="warning" sx={{ mb: 2 }}>You scored {Math.round(quizResult.score)}%. Review your answers and try again when ready.</Alert>}<Alert severity="info" sx={{ mb: 2 }}>Ask TALA can explain the concept or give a hint, but it will not reveal or confirm the quiz answer.</Alert><Stack gap={3}>{selected.practice_questions.map((question, index) => <Box key={question.id} onFocus={() => setFocusedQuestionId(question.id)}><Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 2 }}><Typography variant="body2" fontWeight={700}>{index + 1}. {question.prompt}</Typography><Button size="small" variant="text" onClick={() => { setFocusedQuestionId(question.id); setLearningContext({ contextType: 'learning_material', assignmentId: selected.id, competency: selected.competency?.title ?? 'General learning material', activityTitle: selected.resource_title, questionId: question.id, questionPrompt: question.prompt, selectedAnswer: quizAnswers[question.id], openChat: true }); }}>Ask TALA</Button></Box>{question.question_type === 'short' ? <TextField size="small" fullWidth sx={{ mt: 1.5 }} label="Your answer" value={quizAnswers[question.id] ?? ''} onChange={event => setQuizAnswers(value => ({ ...value, [question.id]: event.target.value }))} disabled={selected.quiz_passed} /> : <FormControl sx={{ mt: 1, width: '100%' }}><RadioGroup value={quizAnswers[question.id] ?? ''} onChange={event => { setFocusedQuestionId(question.id); setQuizAnswers(value => ({ ...value, [question.id]: event.target.value })); }}>{question.options.map(option => <FormControlLabel key={option} value={option} control={<Radio />} label={option} disabled={selected.quiz_passed} />)}</RadioGroup></FormControl>}</Box>)}</Stack></>}</DialogContent><DialogActions><Button onClick={() => { setQuizOpen(false); setQuizStarted(false); }} disabled={busy}>Cancel</Button>{!quizStarted ? <Button variant="contained" onClick={() => { setQuizAnswers({}); setQuizResult(null); setFocusedQuestionId(selected.practice_questions[0]?.id ?? null); setQuizStarted(true); }}>Start Quiz</Button> : !selected.quiz_passed && <Button variant="contained" disabled={busy || !quizAnswered} onClick={() => void submitQuiz()}>{busy ? 'Checking…' : 'Submit Quiz'}</Button>}</DialogActions></Dialog>}
  </>;
}
