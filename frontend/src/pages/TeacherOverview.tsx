...
export function TeacherOverview({ onLearners, onAssessments, onReports, onLearner }: { onLearners: () => void; onAssessments: () => void; onReports: () => void; onLearner: (id: number) => void }) {
  ...
  const context = useTeachingScope();
  useEffect(() => { if (!context?.selectedSubjectId) return; setLearners(null); api<ApiLearner[]>(`/dashboard/teacher/learners/?subject=${context.selectedSubjectId}`).then(setLearners).catch(reason => setError(reason.message)); }, [context?.selectedSubjectId]);
  if ((!learners || !context) && !error) return <Box sx={{ minHeight: 360, display: 'grid', placeItems: 'center' }}><CircularProgress size={28} /></Box>;
  ...
  return <>
    ...
    <Box sx={{ bgcolor: '#173f63', color: '#fff', borderRadius: 1.5, p: { xs: 2.5, md: 3 }, mb: 3, display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'minmax(0, 1fr) auto' }, gap: 3, alignItems: 'center' }}>
      <Box><Typography variant="overline" sx={{ color: 'rgba(255,255,255,.7)', fontWeight: 700 }}>Current teaching scope</Typography><Typography variant="h2" sx={{ mt: .5, color: '#fff' }}>{context?.selectedSubject?.name ?? 'Assigned subject'} · {context?.classes.length ?? 0} classes</Typography><Typography variant="body2" sx={{ color: 'rgba(255,255,255,.76)', mt: .75 }}>{learners?.filter(item => item.assessment !== null).length ?? 0} of {learners?.length ?? 0} assigned learners have submitted an assessment in this subject. {flagged.length} require monitoring or intervention.</Typography></Box>
    ...
    <Card sx={{ mb: 3 }}><Box sx={{ p: 2.5 }}><Typography variant="h2">Assigned Classes</Typography><Typography variant="body2" color="text.secondary" sx={{ mt: .5 }}>Classes available for {context?.selectedSubject?.name ?? 'the selected subject'}.</Typography><Stack divider={<Divider />} sx={{ mt: 1.25 }}>{context?.classes.map(item => <Box key={item.id} sx={{ py: 1.25, display: 'flex', justifyContent: 'space-between', gap: 2 }}><Box><Typography variant="body2" fontWeight={650}>Grade {item.grade_level} – {item.name}</Typography><Typography variant="caption" color="text.secondary">School year {item.school_year}</Typography></Box><Typography variant="caption" color="text.secondary">{item.student_count} students</Typography></Box>)}</Stack>{!context?.classes.length && <Typography variant="body2" color="text.secondary" sx={{ mt: 1.5 }}>No classes are assigned for this subject. Contact an administrator.</Typography>}</Box></Card>
