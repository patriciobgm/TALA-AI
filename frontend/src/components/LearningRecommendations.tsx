...
export function LearningRecommendations({ recommendations, busyKey, onDecision }: Props) {
  return <Card sx={{ mb: 3 }}>
    <Box sx={{ p: 2.5, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 2 }}>
      <Box>
        <Typography variant="overline" color="primary.main" fontWeight={800}>Recommended learning support</Typography>
    ...
    {recommendations.length ? <Stack divider={<Divider />}>{recommendations.slice(0, 6).map(item => {
      ...
      return <Box key={key} sx={{ px: 2.5, py: 2, display: 'grid', gridTemplateColumns: { xs: '1fr', lg: 'minmax(0, 1fr) auto' }, gap: 2, alignItems: 'center' }}>
        <Box sx={{ minWidth: 0 }}>
          <Stack direction="row" gap={1} alignItems="center" flexWrap="wrap"><Typography variant="body2" fontWeight={750}>{item.resource_title}</Typography><StatusChip label={item.confidence === 'limited' ? 'Limited learning data' : `${item.confidence} confidence`} /></Stack>
