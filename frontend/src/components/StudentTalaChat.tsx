...
export function StudentTalaChat() {
  ...
  const elevatedForMaterialHelp = learningContext?.contextType === 'learning_material' && learningContext.openChat && !minimized;
  return <Card ref={chatBox} sx={{ position: 'fixed', zIndex: theme => elevatedForMaterialHelp ? theme.zIndex.modal + 1 : theme.zIndex.drawer + 2, right: { xs: 12, sm: 24 }, bottom: elevatedForMaterialHelp ? { xs: 76, sm: 84 } : { xs: 12, sm: 24 }, width: { xs: minimized ? 236 : 'calc(100vw - 24px)', sm: minimized ? 280 : 370 }, maxHeight: compactScreen ? 'calc(100vh - 100px)' : 'none', overflow: 'hidden', boxShadow: '0 10px 30px rgba(18, 38, 52, .18)', transition: 'width 160ms ease, bottom 160ms ease' }}>
