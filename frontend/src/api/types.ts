...
export interface ApiAssessment { id: number; title: string; subject: number; kind: 'pre' | 'post' | 'remedial'; instructions: string; due_at: string | null; is_active: boolean; available: boolean; availability_reason: string; remaining_activities: number; consent_status?: string; assigned_classes: number[]; created_by: number; question_count: number; competency_ids: number[]; questions?: ApiQuestion[] }
...
export interface MaterialAnalytics {
  ...
  materials: { id: number; title: string; resource_type: string; quiz_question_count: number; assigned: number; not_started: number; in_progress: number; completed: number; quiz_attempted: number; quiz_passed: number; average_quiz_score: number | null }[];
...
export interface LearnerDetail { student: { id: number; name: string; email: string; section: string }; plans: ApiPlan[]; attempts: AssessmentAttempt[]; evidence: LearnerEvidence[]; interventions: { id: number; action: string; note: string; created_at: string }[]; guardians: { id: number; name: string; relationship: string; phone: string; email: string }[]; remedial_exams: { id: number; title: string; eligible: boolean; remaining_activities: number; consent_status: 'not_requested' | 'requested' | 'approved' | 'declined' | 'revoked' | 'expired'; guardian_name: string; requested_at: string | null; evidence_attached: boolean }[]; recommendations: LearningRecommendation[] }
...
export interface ContentImport {
  ...
  extracted_payload: { questions?: ExtractedQuestion[]; question_count?: number; practice_questions?: ExtractedQuestion[]; practice_question_count?: number; description?: string; transcription_status?: 'not_configured' | 'completed'; transcript_character_count?: number };
